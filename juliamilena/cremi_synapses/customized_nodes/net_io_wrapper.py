...
def get_net_output_specs(net, blob_names):
    output_specs = {}
    logging.info('%s blob name' %blob_names[0])
    logging.info('%s in blobs' %net.blobs.keys()[1])
...
class NetIoWrapper:
    ...
    def __init__(self, net):  # name_net_output
        self.net = net
        self.input_specs = get_net_input_specs(net)
        self.output_specs = get_net_output_specs(net, ['bm_presyn_pred'])
        self.inputs = {}
